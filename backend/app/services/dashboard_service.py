...
def child_complete_profile(db: Session, child_id: int) -> ChildCompleteProfileSummaryResponse:
    ...
    return ChildCompleteProfileSummaryResponse(
        child_basic={"id": child.id, "child_id": child.child_id, "full_name": child.full_name, "gender": child.gender, "age": age, "status": child.status, "admission_date": child.admission_date},
        admission_documents={"required_documents_count": len(REQUIRED_DOCUMENT_TYPES), "uploaded_documents_count": docs[0] or 0, "verified_documents_count": int(docs[1] or 0), "admission_complete": (docs[2] or 0) >= len(REQUIRED_DOCUMENT_TYPES)},
        ...
        accommodation={"has_active_bed": bool(bed_row), "building": bed_row[5].building_name if bed_row else None, "block": bed_row[4].block_name if bed_row else None, "floor": bed_row[3].floor_name if bed_row else None, "room": bed_row[2].room_name if bed_row else None, "bed": bed_row[1].bed_name if bed_row else None},
        ...
        education={"has_active_education_record": bool(education), "school_name": education[1].school_name if education else None, "class_level": education[0].class_level if education else None, "academic_year": education[0].academic_year if education else None, "latest_attendance_percentage": float(latest_attendance) if latest_attendance is not None else None, "latest_exam_percentage": float(latest_exam) if latest_exam is not None else None},
